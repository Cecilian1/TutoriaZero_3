...
public:
	Animation(LPCTSTR path, int num, int interval)
	{
		...
		TCHAR path_file[256];//TCAHR 这里应该和 char用法一样
		for (size_t i = 0; i < num; i++)
		{
			_stprintf(path_file, path, i);//_stprintf是将path和i合起来输出到path_file中
...
public:
	...
	bool CheckPlayerCollision(const Player& player)
	{
		...
		bool is_overlap_x = check_position.x >= player_position.x && check_position.x <= player_position.x + PLAYER_WIDTH;
		bool is_overlap_y = check_position.y >= player_position.y && check_position.y <= player_position.y + PLAYER_HEIGHT;
		return is_overlap_x && is_overlap_y;
	}
...
void UpdateBullets(vector<Bullet>& bullet_list, const Player& player)
{
	...
	for (size_t i = 0; i < bullet_list.size(); i++)
	{
		...
		bullet_list[i].position.x = player_position.x + PLAYER_WIDTH / 2 + (int)(radius * cos(radian));
		bullet_list[i].position.y = player_position.y + PLAYER_HEIGHT / 2 + (int)(radius * sin(radian));
	}
}
...
public:
	...
	void Draw()
	{
		...
		case Status::Hovered:
			...
			break;
		case Status::Pushed:
			putimage(region.left, region.right, &img_pushed);
			break;
		}
	}
protected:
